...
from src.services.paineis_manutencao import (
    NOMES_DOS_PAINEIS,
...
class ManutencaoCog(commands.Cog):
    ...
    async def paineis_listar(self, interacao: discord.Interaction):
        ...
        for nome in NOMES_DOS_PAINEIS:
            if nome in nomes_no_banco:
                registro = next(r for r in registros if r.nome_painel == nome)
                linhas.append(
                    f"`✅` **{nome}** · canal `{registro.canal_id}` · msg `{registro.message_id}`"
                )
            else:
                linhas.append(
    ...
    async def paineis_recriar_todos(self, interacao: discord.Interaction):
        ...
        quantidade_ok = sum(1 for r in resultados if r.ok)
        await enviar_card(
            interacao,
    ...
    async def ajuda(self, interacao: discord.Interaction):
        await enviar_card(
            ...
            linhas=[
                ...
                "`/manutencao sincronizar-comandos` — atualiza slash commands no Discord.",
