"""
As gravacoes e as regras da chamada de plantao.

Cuida de abrir a sessao, guardar quem estava presente, e fechar somando o tempo
de cada um. As constantes MOTIVO_CANCEL_* no topo existem para o relatorio dizer
por que uma chamada morreu — tempo esgotado, o doutor saiu do servidor, erro no
processamento — em vez de simplesmente desaparecer sem explicacao.
"""
...
# Motivos de cancelamento de uma sessao de chamada.
# Ficam gravados no banco, por isso o texto nao deve mudar sem migracao.
...
def _limite_apos_rr_mais_recente(agora_utc: datetime) -> datetime:
    ...
    for offset_dias in (0, -1):
        ...
        for horario_str in RR_HORARIOS:
            hora_em_texto, minuto_em_texto = horario_str.split(":")
            hora = int(hora_em_texto)
            minuto = int(minuto_em_texto)
            rr_dt = datetime(
                dia_ref.year,
                dia_ref.month,
                dia_ref.day,
...
async def obter_controle_chamada(session) -> ControleChamada:
    """Obtém o único controle de chamada, criando-o na sessão se ainda não existir.

    Recebe uma sessão já aberta para que o chamador mantenha a operação inteira na
    mesma transação. A criação preguiçosa evita falhas na primeira chamada após uma
    instalação sem exigir que outro código prepare previamente esse registro.
    """
...
async def tentar_iniciar_chamada(doutor_id: int) -> tuple[bool, int | None]:
    """
    Tenta pegar o lock. Retorna (conseguiu,
    id_do_doutor_que_ja_esta_chamando_se_falhou).
...
async def cancelar_chamada(*, motivo: str = MOTIVO_CANCEL_ABANDONO) -> None:
    ...
    registrador.info(f"[chamada] cancelada — motivo={motivo}")
...
async def admin_listar_chamadas(limite: int = 15) -> list[Chamada]:
    """Devolve os registros mais recentes para consulta, sem modificar o histórico."""
    async with async_session() as session:
        resultado = await session.execute(
...
async def admin_buscar_chamada(chamada_id: int) -> Chamada | None:
    """Localiza uma chamada pelo ID permanente ou devolve ``None`` sem lançar erro."""
    async with async_session() as session:
        resultado = await session.execute(
...
async def admin_listar_faltas(
    ...
    """Lista faltas recentes, opcionalmente restritas ao membro indicado.

    Mantém a ordenação decrescente para que o limite mostre os acontecimentos mais
    novos. O filtro usa o ID do Discord, pois nomes e apelidos podem mudar depois que
    a falta já foi registrada no banco.
    """
    async with async_session() as session:
...
async def admin_contar_faltas(discord_id: int) -> int:
    """Conta todas as faltas do membro para exibir um total disciplinar confiável."""
    async with async_session() as session:
        resultado = await session.execute(
            select(func.count())
            .select_from(FaltaChamada)
