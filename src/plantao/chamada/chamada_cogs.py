...
from src.plantao.chamada.chamada_state import definir_sessao
...
class ChamadaCog(commands.Cog):
    ...
    async def on_ready(self):
        ...
        try:
            ...
            if liberou:
                registrador.info(
                    "[chamada] lock expirado liberado no boot (chamada cancelada)"
                )
        except Exception as erro:
            registrador.info(f"[chamada] limpeza de lock no boot: {erro}")
    ...
    async def on_member_remove(self, membro: discord.Member):
        ...
        try:
            ...
            from src.plantao.chamada.chamada_state import obter_sessao
            ...
            if sessao is not None and sessao.doutor_id == membro.id:
                definir_sessao(None)
            registrador.info(f"[chamada] cancelada — doutor {membro.id} saiu da guilda")
        except Exception as erro:
            registrador.info(f"[chamada] on_member_remove: {erro}")
    ...
    async def chamada_status(self, interacao: discord.Interaction):
        """Mostra o estado operacional da próxima chamada.

        Consulta o controle central e envia ao administrador uma resposta temporária
        com a trava, o responsável e o cooldown. Isso permite diagnosticar por que
        uma chamada não pode começar sem modificar os dados do sistema.
        """
        dados = await admin_status_controle()
        ...
        linhas = [
            ...
            f"**Iniciada em:** "
            f"`{formatar_data_hora_local(dados['chamada_iniciada_em'])}"
            f"`",
            f"**Última chamada:** "
            f"`{formatar_data_hora_local(dados['ultima_chamada_em'])}"
            f"`",
            f"**Próximo horário permitido:** "
            f"`{formatar_data_hora_local(dados['proximo_horario'])}"
            f"`",
    ...
    async def chamada_liberar_lock(
        ...
        """Destrava uma chamada interrompida, com cooldown opcional.

        O parâmetro ``marcar_cooldown`` decide se a intervenção também deve registrar
        o intervalo normal entre chamadas. A escolha explícita evita tanto bloquear
        a equipe sem necessidade quanto abrir chamadas seguidas após uma finalização.
        """
    ...
    async def chamada_reset_cooldown(self, interacao: discord.Interaction):
        """Remove o registro de cooldown sem ignorar a janela posterior de RR.

        Altera o controle persistido no banco e confirma a ação no Discord. A regra
        de RR continua ativa porque este ajuste serve para recuperar horários, não
        para burlar todas as restrições de segurança do fluxo.
        """
        ...
        await responder_sucesso(
            ...
            linhas=[
                ...
                "A próxima chamada ainda precisa respeitar a janela pós-RR, se "
                "aplicável.",
            ],
        )
    ...
    async def chamada_historico(
        ...
        """Exibe as últimas chamadas para auditoria administrativa.

        Usa ``limite`` para controlar quantos registros recentes serão mostrados em
        uma resposta temporária, incluindo os totais que ajudam a conferir presença
        e ausência sem editar nem excluir informações do banco.
        """
    ...
    async def chamada_ver(
        ...
        """Detalha uma chamada existente sem depender de mensagens antigas.

        Busca pelo identificador permanente do banco e envia as contagens registradas
        ao administrador. Se o ID não existir, responde com erro em vez de apresentar
        dados parciais que poderiam parecer ser de outra chamada.
        """
    ...
    async def chamada_criar(
        ...
        """Insere uma chamada manual para corrigir uma ausência de registro.

        Grava no banco os totais informados e usa quem executou o comando como doutor
        quando ``doutor`` não for escolhido. Não cria uma sessão interativa, pois
        existe para recuperar fatos passados sem disputar a trava de uma chamada real.
        """
    ...
    async def chamada_excluir(
        ...
        """Apaga um registro de chamada sem apagar faltas relacionadas.

        Executa uma exclusão no banco somente para o ID solicitado e avisa essa
        limitação na resposta. Separar as faltas evita remover automaticamente
        evidências ou punições que precisam ser revisadas de forma consciente.
        """
    ...
    async def chamada_faltas(
        ...
        """Lista faltas recentes, podendo restringi-las a um membro.

        Quando ``membro`` é informado, também consulta o total acumulado dele para
        contextualizar a lista. Apenas envia uma visualização temporária, preservando
        os registros e impedindo que a consulta tenha efeito disciplinar por si só.
        """
    ...
    async def chamada_remover_falta(
        ...
        """Remove do banco uma falta identificada, sem reverter cargos aplicados.

        A exclusão corrige somente o histórico de faltas; punições já atribuídas
        permanecem porque podem ter consequências próprias. A resposta deixa essa
        separação clara para evitar a falsa expectativa de uma reversão completa.
        """
        ...
        await responder_sucesso(
            ...
            linhas=[
                ...
                "Cargos de punição já aplicados **não** são revertidos por este "
                "comando.",
            ],
        )
    ...
    async def chamada_registrar_falta(
        ...
        """Registra uma ausência manual e aciona as regras disciplinares aplicáveis.

        Persiste a falta no banco vinculada a ``chamada_id`` — ou a ``0`` quando não
        há chamada associada — e usa um motivo curto para o histórico. Precisa da
        guilda para que a regra possa avisar ou aplicar cargos ao membro corretamente.
        """
...
async def setup(bot: commands.Bot):
    """
    Adiciona os comandos administrativos de chamada ao bot durante a inicialização.
    """
