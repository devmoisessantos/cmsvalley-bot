...
class AprovacaoView(LoggingViewMixin, discord.ui.LayoutView):
    ...
    async def aprovar(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"Escolha o cargo inicial para {candidato.mention}:",
            view=view_cargo,
            ephemeral=True,
        )
    ...
    async def reprovar(self, interaction: discord.Interaction):
        ...
        cargos_para_remover = [
            c for c in [cargo_estudante, cargo_prova] if c in candidato.roles
        ]
...
class EscolherCargoView(LoggingViewMixin, discord.ui.View):
    ...
    async def escolher(self, interaction: discord.Interaction, item: discord.ui.Select):
        ...
        cargos_remover = [
            c
            for c in [cargo_estudante, cargo_prova, cargo_visitante]
            if c in candidato.roles
        ]
        if cargos_remover:
            await candidato.remove_roles(
        ...
        asyncio.create_task(excluir_mensagem(mensagem, 25))
