...
class BackupManager:
    ...
    def listar_backups(self, guild_id: int) -> list[str]:
        """
        Lê os arquivos JSON da pasta exclusiva de um servidor em ordem decrescente.

        O identificador da guilda separa seus retratos dos de outros servidores e a
        ordenação deixa o backup mais novo na primeira posição, convenção usada por
        comandos que precisam escolher uma opção segura sem receber um nome.
        """
        pasta = self._pasta_do_servidor(guild_id)
        return sorted(
            [nome for nome in os.listdir(pasta) if nome.endswith(".json")],
    ...
    def carregar_backup(self, guild_id: int, nome_arquivo: str) -> dict | None:
        """
        Abre um retrato JSON quando ele pertence ao servidor e existe no disco.

        Recebe o identificador da guilda e o nome escolhido para formar o caminho
        dentro de sua pasta. Retorna o dicionário do backup ou `None` em vez de
        falhar quando o arquivo sumiu, permitindo que o comando explique a ausência.
        """
    ...
    def deletar_backup(self, guild_id: int, nome_arquivo: str) -> bool:
        """
        Apaga do disco um arquivo existente e informa se a remoção foi possível.

        O caminho é limitado à pasta da guilda indicada, o que evita misturar
        retratos de servidores. Retorna `True` somente após excluir o JSON e `False`
        se ele não existir, para que a camada de comando dê uma resposta correta.
        """
        caminho = os.path.join(self._pasta_do_servidor(guild_id), nome_arquivo)
    ...
    def nome_backup_mais_recente(self, guild_id: int) -> str | None:
        """
        Escolhe o primeiro nome ordenado para dar um padrão aos comandos sem arquivo.
        """
        arquivos = self.listar_backups(guild_id)
        return arquivos[0] if arquivos else None

    def caminho_completo(self, guild_id: int, nome_arquivo: str) -> str:
        """Monta o caminho da pasta isolada da guilda sem acessar o arquivo."""
