...
def _limpar_backups_banco_antigos() -> None:
    ...
    for nome_antigo in arquivos[MAX_BACKUPS_PER_GUILD:]:
        ...
        except OSError as erro_em_limpar_backups_banco_antigos:
            # Enfeite que falhou: apagar backups antigos do disco.
            # A acao principal ja tinha dado certo, entao so registro.
            ignorar_falha_cosmetica(
                erro_em_limpar_backups_banco_antigos,
                o_que_falhou="apagar backups antigos do disco",
            )
...
def _tentar_pg_dump(dados_conexao: dict[str, str], caminho_destino: str) -> bool:
    ...
    try:
        ...
        registrador.error(f"[backup-db] pg_dump falhou: {erro}")
        return False

    if resultado.returncode != 0:
        registrador.error(f"[backup-db] pg_dump erro: {resultado.stderr.strip()}")
        if os.path.isfile(caminho_destino):
            try:
                os.remove(caminho_destino)
            except OSError as erro_em_tentar_pg_dump:
                # Enfeite que falhou: rodar o pg_dump.
                # A acao principal ja tinha dado certo, entao so registro.
                ignorar_falha_cosmetica(
                    erro_em_tentar_pg_dump,
                    o_que_falhou="rodar o pg_dump",
                )
        return False
...
async def _exportar_tabelas_json(caminho_destino: str) -> bool:
    ...
    try:
        ...
        registrador.error(f"[backup-db] Import do ORM falhou: {erro}")
        return False
    ...
    try:
        async with async_session() as sessao:
            ...
            except Exception as erro_ao_contar_tabelas:
                # Essa contagem e apenas um carimbo informativo dentro do
                # backup, para depois sabermos com qual formato de banco ele
                # foi gerado. Se falhar, o backup em si continua valido.
                logging.debug(
                    "Nao consegui contar as tabelas para carimbar o backup: %s",
                    erro_ao_contar_tabelas,
                )
    except Exception as erro:
        registrador.error(f"[backup-db] Export JSON falhou: {erro}")
        return False
    ...
    try:
        ...
        registrador.info(f"[backup-db] Não foi possível gravar JSON: {erro}")
        return False
...
async def fazer_backup_banco_se_mudou() -> dict[str, Any]:
    ...
    # O backup mais recente da lista pode ser o arquivo que acabamos de criar,
    # entao o laco abaixo pula o proprio arquivo novo e compara com o anterior.
    arquivos = _listar_backups_banco()
    hash_anterior = None
    ...
    if hash_anterior is not None and hash_novo == hash_anterior:
        ...
        except OSError as erro_em_fazer_backup_banco_se_mudou:
            # Enfeite que falhou: comparar o backup novo com o anterior.
            # A acao principal ja tinha dado certo, entao so registro.
            ignorar_falha_cosmetica(
                erro_em_fazer_backup_banco_se_mudou,
                o_que_falhou="comparar o backup novo com o anterior",
            )
        return {
            "fez_backup": False,
