                    ...
                    try:
                        await sessao.execute(
                            text(
                                ...
                                f"COALESCE((SELECT MAX({coluna.name}) FROM {nome_tabela}), 1))"
                            ),
                            {"tab": nome_tabela, "col": coluna.name},
                        )
                    except Exception:
                        # Nem toda PK é serial — silencioso
                        pass
...
async def sincronizar_banco_com_api() -> dict[str, Any]:
    ...
    if not snapshot_remoto or not (snapshot_remoto.get("tabelas") or {}):
        return {
            "ok": True,
            "motivo": "cofre remoto vazio ou inacessível; push feito se havia dados locais",
            "push": resultado_sync,
            "restore": None,
