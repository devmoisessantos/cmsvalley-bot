...
class ResultadoSincronizacaoUsuarios:
    ...
    def linhas_resumo(self) -> list[str]:
        """Transforma os contadores em linhas prontas para o card de resultado.

        Centraliza a apresentação da sincronização para que o comando informe
        com consistência o que foi criado, alterado ou inferido a partir dos
        cargos, sem repetir o formato da mensagem em cada chamador.
        """
        return [
...
async def sincronizar_usuarios_do_servidor(
    ...
    membros_humanos = [
        membro_do_servidor
        for membro_do_servidor in guild.members
        if not membro_do_servidor.bot
    ]
