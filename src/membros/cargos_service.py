...
async def candidato_elegivel_para_receber_cargo(
    ...
    return (
        ...
        "Membros só com whitelist (**Visitantes**) não podem ser promovidos por este painel.",
    )
...
async def adicionar_cargo(
    ...
    if not elegivel:
        await interaction.followup.send(mensagem_bloqueio, ephemeral=True)
        return
...
async def remover_cargo(
    ...
    if remocoes_recentes is not None:
        ...
        await interaction.followup.send(
            "⚠️ Você está fazendo isso rápido demais. As remoções recentes foram "
