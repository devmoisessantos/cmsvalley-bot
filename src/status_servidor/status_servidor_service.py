...
1. Resolve o IP real pelo cabeçalho x-citizenfx-url do cfx.re/join
   e consulta dynamic.json nesse endpoint.
2. Consulta as URLs de dynamic.json configuradas em STATUS_SERVIDOR.
...
# Último endpoint que respondeu (evita depender só do config se o IP mudar)
_ultimo_endpoint_que_funcionou: str | None = None
...
async def buscar_status_do_servidor() -> dict:
    ...
    - erro (str | None)
    """
    global _ultimo_endpoint_que_funcionou

    urls_para_tentar: list[str] = []

    # 1) Último endpoint que já funcionou nesta execução do bot
    if _ultimo_endpoint_que_funcionou:
        urls_para_tentar.append(_ultimo_endpoint_que_funcionou)

    # 2) Descoberta ao vivo pelo join do CFX
    url_descoberta = await _descobrir_url_dynamic_pelo_join()
    if url_descoberta and url_descoberta not in urls_para_tentar:
        urls_para_tentar.append(url_descoberta)

    # 3) URLs fixas do config
    for url in STATUS_SERVIDOR.get("URLS_DYNAMIC") or []:
        if url and url not in urls_para_tentar:
            urls_para_tentar.append(url)

    for url in urls_para_tentar:
        resultado = await _consultar_uma_url_dynamic(url)
        if resultado is not None:
            _ultimo_endpoint_que_funcionou = url
            registrador.info(
                "Status FiveM OK em %s → %s jogadores",
                url,
                resultado["jogadores"],
            )
            return resultado

    # 4) Último recurso: API pública (costuma 404 neste servidor)
    resultado_cfx = await _consultar_api_cfx()
    if resultado_cfx is not None:
        return resultado_cfx

    registrador.warning(
        "Nenhuma URL de status FiveM respondeu. Tentadas: %s",
        urls_para_tentar,
    )
...
async def _consultar_uma_url_dynamic(url: str) -> dict | None:
    """
    Consulta uma única URL de dynamic.json (ou info.json).
    """
    timeout = aiohttp.ClientTimeout(total=12, connect=6)

    try:
        async with aiohttp.ClientSession(timeout=timeout) as sessao_http:
            async with sessao_http.get(
                ...
                if resposta.status != 200:
                    registrador.warning(
                        "Status HTTP %s ao consultar %s",
                        resposta.status,
                        url,
                    )
                    return None

                # Alguns hosts não mandam content-type JSON
                try:
                    dados = await resposta.json(content_type=None)
                except Exception:
                    texto = await resposta.text()
                    registrador.warning(
                        "Resposta não-JSON de %s: %s",
                        url,
                        texto[:120],
                    )
                    return None
                ...
                if not isinstance(dados, dict):
                    registrador.warning(
                        "JSON inesperado de %s: tipo %s",
                        url,
                        type(dados).__name__,
                    )
                    return None

                if "clients" not in dados and "sv_maxclients" not in dados:
                    registrador.warning(
                        "JSON de %s sem campos de jogadores: %s",
                        url,
                        list(dados.keys())[:8],
                    )
                    return None
    ...
    except aiohttp.ClientError as erro_http:
        registrador.warning(
            "Erro de rede ao consultar %s: %s",
            url,
            erro_http,
        )
        return None
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as sessao_http:
            async with sessao_http.get(
                ...
                if not endpoint:
                    registrador.warning(
                        "Join CFX %s não trouxe x-citizenfx-url (status HTTP %s).",
                        codigo,
                        resposta.status,
                    )
                    return None
                ...
                registrador.info(
                    "Endpoint real do servidor via join: %s",
                    endpoint,
                )
                return url_dynamic
    except Exception as erro_capturado:
...
async def _consultar_api_cfx() -> dict | None:
    ...
    try:
        async with aiohttp.ClientSession(timeout=timeout) as sessao_http:
            async with sessao_http.get(url) as resposta:
                if resposta.status != 200:
                    registrador.debug(
                ...
                dados_completos = await resposta.json(content_type=None)
